...
double Z_BOARD = 0.000; // Board height constant
...
int main(int argc, char **argv) {
  ...
  // Reset flag indicating completion of data collection
  global_variable::flag_done_collecting = false;
  // Initialize the robot, model, and gripper with the provided hostname
...
// Attempt to parse the coordinate values from the command line arguments
...
// Set the robot's target location based on the provided coordinates
  set_location(x, y);

  std::cout << "The robot will now grasp the object, press Enter to continue..." << std::endl;
  std::cin.ignore();
  
  open_gripper();// Open the gripper before moving to the object
